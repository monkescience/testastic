...
func Len(t testing.TB, collection any, expected int) {
	...
	if actual == -1 {
		t.Errorf("testastic: assertion failed\n\n  Len\n    error: cannot get length of %T", collection)
		return
	}
	if actual != expected {
		t.Errorf("testastic: assertion failed\n\n  Len\n    expected: %s\n    actual:   %s", red(fmt.Sprintf("%d", expected)), green(fmt.Sprintf("%d", actual)))
	}
}
...
func Empty(t testing.TB, collection any) {
	...
	if length != 0 {
		t.Errorf("testastic: assertion failed\n\n  Empty\n    expected: %s\n    actual:   %s", red("empty (length 0)"), green(fmt.Sprintf("length %d", length)))
	}
}
...
func NotEmpty(t testing.TB, collection any) {
	...
	if length == 0 {
		t.Errorf("testastic: assertion failed\n\n  NotEmpty\n    expected: %s\n    actual:   %s", red("non-empty"), green("empty (length 0)"))
	}
}
...
func SliceContains[T comparable](t testing.TB, slice []T, element T) {
	...
	t.Errorf("testastic: assertion failed\n\n  SliceContains\n    slice:   %s\n    element: %s (not found)", green(formatSlice(slice)), red(formatVal(element)))
}
...
func SliceNotContains[T comparable](t testing.TB, slice []T, element T) {
	...
	for _, v := range slice {
		if v == element {
			t.Errorf("testastic: assertion failed\n\n  SliceNotContains\n    slice:   %s\n    element: %s (found)", green(formatSlice(slice)), red(formatVal(element)))
			return
		}
	}
}
...
func SliceEqual[T comparable](t testing.TB, expected, actual []T) {
	...
	if len(expected) != len(actual) {
		t.Errorf("testastic: assertion failed\n\n  SliceEqual\n    expected: %s (length %d)\n    actual:   %s (length %d)", red(formatSlice(expected)), len(expected), green(formatSlice(actual)), len(actual))
		return
	}
	for i := range expected {
		if expected[i] != actual[i] {
			t.Errorf("testastic: assertion failed\n\n  SliceEqual\n    expected: %s\n    actual:   %s\n    diff at index %d: %s != %s", red(formatSlice(expected)), green(formatSlice(actual)), i, red(formatVal(expected[i])), green(formatVal(actual[i])))
			return
		}
	}
}
...
func MapHasKey[K comparable, V any](t testing.TB, m map[K]V, key K) {
	t.Helper()
	if _, ok := m[key]; !ok {
		t.Errorf("testastic: assertion failed\n\n  MapHasKey\n    map:  %s\n    key:  %s (not found)", green(formatMap(m)), red(formatVal(key)))
	}
}
...
func MapNotHasKey[K comparable, V any](t testing.TB, m map[K]V, key K) {
	t.Helper()
	if _, ok := m[key]; ok {
		t.Errorf("testastic: assertion failed\n\n  MapNotHasKey\n    map:  %s\n    key:  %s (found)", green(formatMap(m)), red(formatVal(key)))
	}
}
...
func MapEqual[K comparable, V comparable](t testing.TB, expected, actual map[K]V) {
	t.Helper()
	if len(expected) != len(actual) {
		t.Errorf("testastic: assertion failed\n\n  MapEqual\n    expected: %s (length %d)\n    actual:   %s (length %d)", red(formatMap(expected)), len(expected), green(formatMap(actual)), len(actual))
		return
	}
	for k, ev := range expected {
		av, ok := actual[k]
		if !ok {
			t.Errorf("testastic: assertion failed\n\n  MapEqual\n    expected: %s\n    actual:   %s\n    missing key: %s", red(formatMap(expected)), green(formatMap(actual)), red(formatVal(k)))
			return
		}
		if ev != av {
			t.Errorf("testastic: assertion failed\n\n  MapEqual\n    expected: %s\n    actual:   %s\n    diff at key %s: %s != %s", red(formatMap(expected)), green(formatMap(actual)), formatVal(k), red(formatVal(ev)), green(formatVal(av)))
			return
		}
	}
}
